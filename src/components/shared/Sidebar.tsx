...
const navByRole: Record<string, NavItem[]> = {
  ...
  water_admin: [
    ...
    { label: 'Stock Requests', path: '/water/admin/stock-requests', icon: <FileText size={18} /> },
    ...
    <div className={`flex flex-col h-full ${theme.gradient} text-white`}>
      ...
      <div className="p-5 border-b border-white/10">
        <div className="flex items-center gap-3 mb-1">
          <div className={`w-9 h-9 ${theme.accent} rounded-lg flex items-center justify-center shadow-lg`}>
            {theme.icon}
          </div>
          <div>
            <p className="font-bold text-sm leading-tight" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
              JAKE'S ENTERPRISE
            </p>
            <p className="text-white/60 text-xs">{theme.label}</p>
          </div>
        </div>
      </div>
